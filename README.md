...
A React Native library that renders an emoji into a native bitmap, extracts its dominant color, real darker and lighter representative colors, and can optionally return a small palette.
...
`mainDarkColor` and `mainLightColor` are selected from the same sampled native emoji clusters as `mainColor`. They are not generated by darkening or lightening `mainColor`.
...
6. Return the top cluster as `mainColor`, the strongest real darker and lighter clusters as `mainDarkColor` and `mainLightColor`, and the top `paletteSize` clusters as `palette`.
